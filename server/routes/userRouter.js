...
// Helper to get JWT secret (support both JWT_SECRET and JWT_SECRET_KEY env names)
...
router.post('/signup', async (req, res, next) => {
  try {
    ...
    if (!JWT_SECRET) {
      const err = new Error('JWT secret is not set on server (JWT_SECRET or JWT_SECRET_KEY)')
      err.status = 500
      return next(err)
    }
    ...
    // Unique violation for email in Postgres -> code '23505'
    if (err.code === '23505') {
...
router.post('/signin', async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query('SELECT * FROM account WHERE email = $1', [user.email])
    if (!rows.length) {
